...
"""
    voros_symbol(w::WKBExpansion, contour; closed = true, atol = nothing) -> VorosSymbol
...
`atol`, default scaled to the period magnitudes) and stored as exact zero.
"""
function voros_symbol(w::WKBExpansion, contour::AbstractVector; closed::Bool = true,
    ...
    classical = wkb_period(w, contour, -1; closed)
    F = typeof(real(classical))
    v = Complex{F}[wkb_period(w, contour, m; closed) for m in 1:w.order]
