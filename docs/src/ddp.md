...
a singularity - a Stokes ray - the two lateral sums differ, and the difference is not arbitrary:
...
with ``\langle\gamma,\gamma'\rangle`` the intersection pairing of the two cycles - an *integer*.
That is the whole point of the layer: the Stokes constant is exact combinatorial data, while
the summed values on either side are numerical. [`verify_ddp`](@ref) measures the exponent from
the two lateral sums and returns it next to its residual, so the integer is a result, not an
input.
...
fails the check by an order of magnitude, so this is a real test of orientation, not a
tautology.

The Voros symbols themselves never move. Crossing ``\theta_c`` only rotates the Laplace ray in
the Borel plane; the jump is a property of the summation, not of the series.
...
(``\mathrm{Re}(Z e^{-i\theta_c}) < 0``); a reversed one raises a typed error instead of being
silently flipped.
