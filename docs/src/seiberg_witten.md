...
Pure ``SU(2)`` ``\mathcal N = 2`` gauge theory is the standard exact
Wentzel-Kramers-Brillouin problem. Its Nekrasov-Shatashvili limit is the Mathieu equation,
which in this package's normalization ``\hbar^2\psi'' = (V-E)\psi`` reads
...
This layer computes them directly as period integrals and not through the polynomial
recursion engine, since ``\cos 2x`` is not a polynomial. It is deliberately firewalled:
...
where ``a_D \to 0``, the dyon point ``u = -2\Lambda^2``, and the weak-coupling puncture at
infinity. Encircling any of them permutes the periods by an integer symplectic matrix
...
``S_2 \sim Q^{-5/2}`` is not pointwise integrable, yet no Voros regularization is needed.
...
The Bogomolny-Prasad-Sommerfield (BPS) quiver of pure ``SU(2)`` is the Kronecker quiver, which
is affine ``\tilde A_1`` and therefore cluster-*infinite*. That is the combinatorial reason the
weak-coupling spectrum is an infinite tower and not a finite list. [`su2_bps_states`](@ref)
returns the spectrum in either chamber, resolved automatically from the wall of marginal
stability ([`ms_wall`](@ref), [`sw_chamber`](@ref)):
...
is self-validating: closure to a given degree pins the spectrum content, the ``\Omega`` values
including the vector multiplet's ``-2``, and the ordering all at once, with no external answer
supplied.
