...
instead of returning. A graph with a saddle connection is not a generic graph, a coalescing
pair of turning points is not a spectral cycle, and a non-decaying wall symbol is not a wall
symbol. In each case the alternative would be a plausible-looking number that means nothing.
